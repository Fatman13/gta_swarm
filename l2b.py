	...
	with open(file) as fp:
		...
		for tr_ele in soup.find_all('tr'):
			...
			for i, td_ele in enumerate(tr_ele.find_all('td')):
				...
				if i == 2: 
					if td_ele.get_text() != '-':
						entry[date] = float('{0:.0f}'.format(float(searches / int(td_ele.get_text().replace(',','')) / 7)))
					if td_ele.get_text() == '-':
...
with open('output_l2b_weekly_' + datetime.datetime.now().strftime('%y%m%d_%H%M') + '.csv', 'w', encoding='utf-8') as output_file:
