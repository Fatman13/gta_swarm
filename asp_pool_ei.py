...
def asp(s_request):
	...
	for i in range(MAX_RETRIES):
		try:
			r = requests.post(url, data=s_request['body'], timeout=15)
			...
			print('PPID: {} PID: {} GTA_key: {}'.format(os.getppid(), os.getpid(), str(s_request['GTA_city'])) )
		except OSError:
...
@click.command()
# @click.option('--file_name', default='gta_city')
@click.option('--file_name', default='gta_hotel_codes')
@click.option('--checkin_d', default='2018-04-21')
# @click.option('--to_d', default='2017-11-20')
@click.option('--client', default='mogu')
def asp_pool_ei(file_name, checkin_d, client):
	res = []
	search_requests = []
	...
	with open(file_name, 'r') as file:
		for line in file:
			...
			try:
				...
				continue
			# hotel_codes.append(dict([('city_code', line.rstrip()), ('item_code', '?')]))
	...
	search_tree = ET.parse(os.path.join(os.getcwd(), 'SearchHotelPricePaxRequest_EI.xml'))
	...
	for counter, hotel_code in enumerate(hotel_codes):
		...
		ent = {}
		ent['GTA_city'] = '_'.join([hotel_code['city_code'], hotel_code['item_code']])
		# ent['GTA_city'] = hotel_code['city_code']
	...
	for response in responses:
		...
		for hotel in r_tree.find('.//HotelDetails'):
			...
			# print(hotel_name)
			# print(r_tree.find('.//RoomCategories').text)

			for room_cat in hotel.find('.//RoomCategories'):
				# print(room_cat.text)
				# print(str(room_cat.find('.//EssentialInformation') == None))
				# EssentialInformation
				if room_cat.find('.//EssentialInformation') == None:
					continue
				# print('Id: ' + str(room_cat.get('Id')))
				# print('Des: ' + str(room_cat.find('.//Description').text))

				for ei in room_cat.find('.//EssentialInformation'):
					entry = {}
					entry['GTA_key'] = gta_key
					entry['Hotel_Name'] = hotel_name
					entry['EI'] = ei.find('.//Text').text
					# print(entry)
					res.append(entry)
				break
	...
	# print(res)
	if not res:
		print('Warning: Empty res...')
		return

	output_file_name = '_'.join([ 'Output_search_price_ei',
									client,
									file_name,
