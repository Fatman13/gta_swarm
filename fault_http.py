...
def fault():
				...
				for i, tr_ele in enumerate(soup.find_all('tr')):
					...
					for j, td_ele in enumerate(tr_ele.find_all('td')):
						...
						if HTTP in file.lower():
							...
							# 		ent['type'] = 'add'
							# 	elif 'searchhotelprice' in td_ele.get_text().lower():
							# 		ent['type'] = 'search'
							# 	else:
							# 		ent['type'] = 'unknown'
							# if j == 2:
	...
	# 				entry = {}
	# 				entry['name'] = '_'.join([client, data1['type'], 'failure_http'])
	...
	for client in clients:
		if client not in res_error_dict.keys():
			continue
		entry = {}
		entry['name'] = '_'.join([client, 'failure_http'])
		for data1 in res_error_dict[client]:
			entry[data1['date']] = data1['value']
		res.append(entry)
	...
	# 	for key in ent.keys():
	# 		field_names.add(key)
	# 	# field_names.add(list(ent.keys()))
	# field_names.remove('name')
	# field_names = list(field_names)
	# field_names = sorted(field_names, key=lambda x: datetime.datetime.strptime(x, '%d %b %Y'))
	# field_names.append('name')
	print(res)
	traget_filename = '_'.join(['output_failure_http_weekly', datetime.datetime.now().strftime('%y%m%d_%H%M')]) + '.csv'
